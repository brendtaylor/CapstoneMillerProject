...
docker exec -i sql-server-db /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'TestPassword!2025' -C -N -Q "CREATE DATABASE TICKET_SYSTEM"

# Step 3: Restart API container
docker-compose restart api

# Step 4: Copy and run schema script
docker cp "Database/createDatabaseAndTables" sql-server-db:/tmp/createDb.sql
